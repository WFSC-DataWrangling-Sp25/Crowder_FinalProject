...
```{r}
library(tidyverse) 
#Original_data <- read_tsv("../Final Project Data/data_raw/e249_Plant aboveground biomass data.txt") 

#REVIEW: changed file path
Original_data <- read_tsv("data_raw/e249_Plant aboveground biomass data.txt")

#REVIEW: This file path works better because your Rmd file is in the same folder as your working directory so you don’t need the ../Final Project Data/ part. However, I would recomend moving your final project script into your scripts folder and then the filepath that you used would work.

#REVIEW: Also I recommend not having spaces in your working directory name and also naming this Rmd file something other than "Script" such as "Final_Project_script".
```
...
#REVIEW: you could have a different df name like surveys or plant or something other than data raw so it doesn’t get confusing
...
#REVIEW: you can do this all in one rename function like this ex:

#rawdata <- rawdata %>% 
#  rename(
#    Sample_date = 'Sample Date',
#    Heat_Treatment = 'Heat Treatment',
#    Mass_g_per_m2 = `Mass (g/m2)`,
#    Species_name = 'Species'
#  )

#REVIEW: also if you wanted to you could do more consistent capitalization to make things easier ex:
...
#REVIEW: I would save this into your data clean folder at this point you didnt have one so I made one 
write_csv(sepdata, "data_clean/cleaned_sepdata.csv")
```
...
# Create a graph showing the relative amount of each species
Count_sepdata <- sepdata %>%
  group_by(Genus, Species) %>% 
  unite("United", c("Genus", "Species"), sep = " " )

#REVIEW: I would maybe call the column united_species or something more specific because just united may be confusing later on when you look back at the code

species_counts <-  ggplot(Count_sepdata)+ 
  geom_bar(aes(x = United), linewidth = 12)+
  theme(axis.text.x = element_text(angle = 90, hjust = 0.5))+ # used Chatgpt here
  labs(x = "Species", y = "Number of Instances")

# The above information as an image using a relative path
ggsave("outputs/species_counts.png", height = 7, width = 12)

#REVIEW: I changed size to linewidth because ggplot told me to 

#REVIEW: I made this plot because I feel like the readability is better, I set it to increasing number of counts rather than alphabetical, not sure if this will be useful to you 
...
ggsave("outputs/species_counts2.png", width = 8, height = 12)

#REVIEW: here is the same plot without the coords flipped 
species_counts3 <- ggplot(Count_sepdata2, aes(x = fct_reorder(United, n), y = n)) +
  ...
  labs(x = "Species", y = "Number of Instances", title = "Species Occurrence Counts")
 
 ggsave("outputs/species_counts3.png", width = 12, height = 8)
```
...
ggplot(Mass_sepdata, aes(x = United, y = lbs_per_acre, fill = Heat_Treatment))+
  geom_bar(stat = "identity")+
  theme(axis.text.x = element_text(angle = 90, hjust = 0.5))+ # used Chatgpt here
  labs(x = "Species", y = "Pounds per Acre")
  
# Code that saves the ggplot into the Output Folder using a relative path
ggsave("../Final Project Data/outputs/species_mass.png", height = 7, width = 10)

#REVIEW: i changed the file path from "../Final Project Data/output/species_mass.png" to "outputs/species_mass.png" because your markdown folder is in your working director, so it doesn’t need the "../Final Project Data"

####################################################################################

#REVIEW: I made this plot from your code, its not that different I just think the readability is a bit better 

Mass_sepdata2 <- Mass_sepdata %>%
  mutate(United = gsub(" ", "\n", United))

ggplot(Mass_sepdata2, aes(x = United, y = lbs_per_acre, fill = Heat_Treatment)) +
  geom_bar(stat = "identity") +
  theme_classic() +
